...
class OrganizationInformationService:
    ...
    @staticmethod
    def get_one(id: int):
        return db.query(OrganizationInformation).filter(OrganizationInformation.id == id).first()
    
    @staticmethod
    def update_one(id: int, **kwargs):
        try:
            result = db.query(OrganizationInformation).filter(OrganizationInformation.id == id).update(kwargs)
            db.commit()
            return result
